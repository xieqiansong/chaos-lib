        ...
        <el-card class="sdk-card" shadow="hover">
          <template #header>
            <div class="card-header">
              ...
              <el-tag v-if="!defEnabled(type)" size="small" type="info">已禁用</el-tag>
            </div>
          </template>

          <!-- 来源列表 -->
          <div class="sources">
            <div v-for="(src, idx) in defSources(type)" :key="idx" class="source-item">
              <el-tag size="small" :type="src.kind === 'repo' ? 'primary' : 'warning'">{{ src.kind }}
              </el-tag>
        ...
        <el-table-column label="操作" width="160">
          <template #default="{ row }">
            <div class="op-actions">
              <el-button size="small" text @click="editDef(row)">编辑</el-button>
              <el-button size="small" type="danger" text @click="removeDef(row)">删除</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
