...
const (
	FsrsStateNew        FsrsState = 0
	FsrsStateLearning   FsrsState = 1
	FsrsStateReview     FsrsState = 2
	FsrsStateRelearning FsrsState = 3
)

type FsrsRating int

const (
	FsrsRatingAgain FsrsRating = 1
	FsrsRatingHard  FsrsRating = 2
	FsrsRatingGood  FsrsRating = 3
	FsrsRatingEasy  FsrsRating = 4
)
...
func (f *Fsrs) forgettingCurve(elapsedDays int, stability float64) float64 {
	decay, factor := f.computeDecayFactor()
	stability = math.Max(stability, sMin)
...
func (f *Fsrs) nextDifficulty(d float64, grade FsrsRating) float64 {
	...
	return clamp(f.meanReversion(f.initDifficulty(FsrsRatingEasy), nextD), 1, 10)
}

func (f *Fsrs) nextRecallStability(d, s, r float64, grade FsrsRating) float64 {
	s = math.Max(s, sMin)
	hardPenalty := 1.0
	if grade == FsrsRatingHard {
		hardPenalty = f.params.Weights[15]
	}
	easyBonus := 1.0
	if grade == FsrsRatingEasy {
		easyBonus = f.params.Weights[16]
	}
	return clamp(s*(1+math.Exp(f.params.Weights[8])*(11-d)*math.Pow(s, -f.params.Weights[9])*(math.Exp((1-r)*f.params.Weights[10])-1)*hardPenalty*easyBonus), sMin, sMax)
}

func (f *Fsrs) nextForgetStability(d, s, r float64) float64 {
	return clamp(f.params.Weights[11]*math.Pow(d, -f.params.Weights[12])*(math.Pow(s+1, f.params.Weights[13])-1)*math.Exp((1-r)*f.params.Weights[14]), sMin, sMax)
}
...
func (f *Fsrs) nextShortTermStability(s float64, grade FsrsRating) float64 {
	...
	maskedSinc := sinc
	if grade >= FsrsRatingGood {
		maskedSinc = math.Max(sinc, 1)
	}
	return clamp(s*maskedSinc, sMin, sMax)
}
...
func (f *Fsrs) getFuzzRange(ivl, elapsedDays int) (int, int) {
	...
	for _, r := range fuzzRanges {
		delta += r.factor * math.Max(math.Min(float64(ivl), r.end)-r.start, 0)
	}
...
func basicLearningStepsStrategy(params *FsrsParameters, state FsrsState, curStep int) map[FsrsRating]learningStepResult {
	var steps []string
	if state == FsrsStateRelearning || state == FsrsStateReview {
	...
	if state == FsrsStateReview {
		stepInfo := steps[maxInt(0, curStep)]
		result[FsrsRatingAgain] = learningStepResult{convertStepToMinutes(stepInfo), 0}
		return result
	}
	result[FsrsRatingAgain] = learningStepResult{getAgainInterval, 0}
	result[FsrsRatingHard] = learningStepResult{getHardInterval, curStep}
	if curStep+1 < stepsLength {
		nextMin := convertStepToMinutes(steps[curStep+1])
		result[FsrsRatingGood] = learningStepResult{int(math.Round(float64(nextMin))), curStep + 1}
	}
	return result
}
...
func (f *Fsrs) Next(card *FsrsCard, now time.Time, rating FsrsRating) *FsrsSchedulingResult {
	elapsedDays := 0
	if card.State != FsrsStateNew && card.LastReview != nil {
...
func (f *Fsrs) newState(card *FsrsCard, now time.Time, rating FsrsRating, elapsedDays, reps int, lastReview time.Time, rng *rand.Rand) *FsrsSchedulingResult {
	...
	f.applyLearningSteps(&next, card, now, rating, FsrsStateLearning, rng)
...
func (f *Fsrs) reviewState(card *FsrsCard, now time.Time, rating FsrsRating, elapsedDays, reps int, lastReview time.Time, rng *rand.Rand) *FsrsSchedulingResult {
	...
	nextAgain.Difficulty = f.nextDifficulty(d, FsrsRatingAgain)
	nextHard.Difficulty = f.nextDifficulty(d, FsrsRatingHard)
	nextGood.Difficulty = f.nextDifficulty(d, FsrsRatingGood)
	nextEasy.Difficulty = f.nextDifficulty(d, FsrsRatingEasy)

	nextSMin := s / math.Exp(f.params.Weights[17]*f.params.Weights[18])
	sAfterFail := f.nextForgetStability(d, s, r)
	nextAgain.Stability = clamp(nextSMin, sMin, sAfterFail)
	nextHard.Stability = f.nextRecallStability(d, s, r, FsrsRatingHard)
	nextGood.Stability = f.nextRecallStability(d, s, r, FsrsRatingGood)
	nextEasy.Stability = f.nextRecallStability(d, s, r, FsrsRatingEasy)
	...
	nextHard.State = FsrsStateReview
	nextHard.LearningSteps = 0
	nextGood.State = FsrsStateReview
	nextGood.LearningSteps = 0
	nextEasy.State = FsrsStateReview
	nextEasy.LearningSteps = 0

	f.applyLearningSteps(&nextAgain, card, now, FsrsRatingAgain, FsrsStateRelearning, rng)
	nextAgain.Lapses++

	var resultCard FsrsCard
	switch rating {
	case FsrsRatingAgain:
...
func (f *Fsrs) getLearningInfo(card *FsrsCard, rating FsrsRating) (scheduledMinutes, nextSteps int) {
	...
	if card.State == FsrsStateLearning && rating != FsrsRatingAgain && rating != FsrsRatingHard {
